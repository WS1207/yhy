...
router.get('/getBanner',(req,res)=>{
    var sql="select * from culture_cate";
...
router.get('/addBanner', (req, res)=> {
    var sql="insert into culture_cate (id,title_url) values (null,'') ";
    mysql.query(sql,[], function (err,data) {
        if(err){
            console.log(err)
        }else{
...
router.post('/delBanner', (req, res)=> {
    var sql="delete from culture_cate where id=?"
...
router.post('/article_sele', (req, res) => {
    mysql.query('select * from culture_article', [], function (data) {
        res.json(data);
    })
});
//刪除
router.post('/article_del', (req, res) => {
    mysql.query('delete from culture_article where id = ?',
...
router.post('/article_add', (req, res) => {
    mysql.query('INSERT INTO culture_article (title,content,url) VALUES (?,?,?) ', [req.body.title,req.body.content,req.body.url], function (data) {
        res.json('ok');
    })
});
//修改
router.post('/article_data',(req,res)=>{
    if(req.body.index==`${req.body.index}`){
        mysql.query(`UPDATE culture_article SET ${req.body.index}=? where id=?`, [req.body.value,req.body.id],function (data) {
            // console.log(data)
        })
    }
})
